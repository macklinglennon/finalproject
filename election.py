...
def state_edges(election_result_rows):
    ...
    for item in election_result_rows:
        state = item["State"]
        dem = item["Dem"]
        rep = item["Rep"]
        demf = float(dem)
...
def most_recent_poll_row(poll_rows, pollster, state):
    ...
    for item in poll_rows:
        if state == item["State"] and pollster == item["Pollster"]:
            poll_state_row.append(item)

    length_poll_state_row = len(poll_state_row)

    if length_poll_state_row == 0:
       return None
    elif length_poll_state_row == 1:
       return poll_state_row[0]

    while length_poll_state_row > 1:
        date1 = poll_state_row[0]["Date"]
        date2 = poll_state_row[1]["Date"]
        if earlier_date(date1, date2):
            poll_state_row.remove(poll_state_row[0])
        else:
            poll_state_row.remove(poll_state_row[1])                                 
        length_poll_state_row = len(poll_state_row)  

    PollDataRow = poll_state_row[0]    
    return PollDataRow
    pass
...
def unique_column_values(rows, column_name):
    ...
    for item in rows:
        ...
        AllRows.append(column)

    unique_name = set(AllRows)
    return unique_name    
    pass
...
def pollster_predictions(poll_rows):
    ...
    for item in poll_rows:
        ...
        StateEdge = state_edges([item])
        PollsterPrediction[pollster] = StateEdge
        print(PollsterPrediction[pollster])
    pass
